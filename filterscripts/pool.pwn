...
new PlayingPool[MAX_PLAYERS],PoolCamera[MAX_PLAYERS],UsingChalk[MAX_PLAYERS],PoolScore[MAX_PLAYERS],Float:AimAngle[MAX_PLAYERS][2],AimObject,\
	PoolStarted,PoolAimer = -1,PoolLastShooter = -1,PoolLastScore,PoolBall[16][poolBall],Text:PoolTD[4],Float:PoolPower,PoolDir;
...
CMD:louerbillard(playerid,params[])
{
	if (!IsPlayerInRangeOfPoint(playerid, 5.0, 511.8775,-76.5869,998.7578))
	    return SendClientMessage(playerid, COLOR_LIGHTRED,"Vous devez ¸etre proche du point de location.");
	//if(GetPlayerMoney(playerid) <= 10) return GameTextForPlayer(playerid, "~r~Vous n'avez pas assez d'argent", 2000, 3);
	SendClientMessage(playerid, COLOR_LIGHTRED,"Vous avez louer la queue de billard au cout de 10$.");
	SendClientMessage(playerid, COLOR_LIGHTRED,"Faites /pool pour commencer a jouer sur la premiï¿½re table un ding! se fera attendre.");
...
CMD:delouerbillard(playerid,params[])
{
	...
	SendClientMessage(playerid, COLOR_LIGHTRED,"Vous avez fini votre location de la queue de billard. Voila vos 10$.");
	//GiveMoney(playerid, 10);
