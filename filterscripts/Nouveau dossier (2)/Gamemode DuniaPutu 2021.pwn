...
#include <colandreas> // first for objecting
#include <3DTryg> // for select vehicle

#include <VehPara> // parachute on the vehicle
...
// ######### VALUE VARIABLE OF RGB FOR EACH COLOR #########

#define COL_EASY "{FFF1AF}"
#define COL_WHITE "{FFFFFF}"
#define COL_BLACK "{0E0101}"
#define COL_GREY "{C3C3C3}"
#define COL_GREEN "{AAFFAA}"
#define COL_RED "{FF4466}"
#define COL_YELLOW "{FFFF77}"
#define COL_ORANGE "{FFAF00}"
#define COL_LIME "{BBFFBB}"
#define COL_CYAN "{00FFEE}"            
#define COL_LIGHTBLUE "{00C0FF}"
#define COL_BLUE "{00AAEE}"
...
// ######### VALUE VARIABLE OF RGBA COLOR ANDROID SYSTEM Android (android.graphics.Color) #########

#define COLOR_ME     0xAAAAFFFF //0xC2A2DAFF
#define COLOR_ERROR   0x888888FF
#define COLOR_GREEN   0xAAFFAAFF
#define COLOR_AQUAGREEN     0x03D687AA
#define COLOR_RED 0xFFAAAAFF //0xFF0000FF
#define COLOR_LIGHTRED 0xFF6347AA // 0xB33C3CAA
#define COLOR_LIGHTBLUE 0x33CCFFAA
#define COLOR_BLUE 0xFF9999FF //0x1ED5C7FF
#define COLOR_YELLOW 0xFFFF99AA
...
#define COLOR_LGREEN 0xAAFFAAAA
#define COLOR_GRAY 0xFF676767
#define COLOR_DARKGOLD 0x808000AA
#define COLOR_AQUA 0x00FFFFFF
#define COLOR_RUSAK 0xFF9A9A9A //0x01FCFFC8
...
// ######### VALUE VARIABLE OF CHAT TALKS SYSTEM #########
...
#include "mapping/LosSantosApartment.inc" 
...
SEM( playerid, ""COL_BLUE"PARACHUTE:"COL_WHITE" Sir now has been thrown");
return 1;
}
...
SEM( playerid, ""COL_BLUE"PARACHUTE:"COL_WHITE" Sir now has been succes to opened");
return 1;
}
...
SEM( playerid, ""COL_BLUE"PARACHUTE:"COL_WHITE" Sir sorry you are cant use parachute on this vehicle");
return 1;
}
//#include "source/function.pwn" 
...
public OnGameModeInit()
{
  ...
  */
  ...
  TextDrawBoxColor(TeksSaatLogin, COLOR_LGREEN);
  ...
  */
  ...
  TextDrawSetOutline(TeksSaatLogin, 0);
  TextDrawBackgroundColor(TeksSaatLogin, COLOR_WHITE);
  ...
  */
  ...
  total_vehicles_from_files += LoadStaticVehiclesFromFile("vehicles/pilots.txt");
    ...
    printf("Semuanya Sir sudah muat jumlahnya %d kendaraan",total_vehicles_from_files);
  ...
  */
    ...
    new GZOnHospital = GangZoneCreate(Float:-1878.5935 , Float:1095.0109, Float:-1542.2660, Float:842.9433);
    //GangZoneShowForPlayer(playerid, zone, )
    GangZoneShowForAll(GZOnHospital,  0x00800099); // change color ARGB transparan with number after blue color

    //GangZoneFlashForAll(GZOnHospital, COLOR_WHITE);
  
  //new GZOnHospital = CreateDynamicRectangle(Float:-1878.5935 , Float:1095.0109, Float:-1542.2660, Float:842.9433, 0, 0,  -1,  0);
  ...
  TextDrawBoxColor(PerlihatkanBoombox, COLOR_LBLUE);
...
public OnPlayerSpawn(playerid)
{
  ...
  if(StatusBaruLogin[playerid] == 1)
    ...
    new loaddataplayer[200];
      ...
      GameTextForPlayer(playerid, "LOADING FROM MYSQL~n~~r~~h~Please Await Sir", 6000, 3);
...
{ 
  ...
  */
  ...
  for( new w = 0; w < 4; w++)
  {
    ...
    PlayerTextDrawUseBox(playerid, teksenergy, 0);
    PlayerTextDrawBoxColor(playerid, teksenergy, COLOR_WHITE);
...
public OnPlayerDisconnect(playerid, reason)
{
  ...
  */
    ...
    printf("The vehicle health %i stored is %f", VehiclePlayerPrimary[playerid][VehID], Float:healthmp1);
    SavePlayer(playerid);
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
  ...
  if(dialogid == DIALOG_OPTION_ATTACHED_DYNAMIC_OBJECT)
  {
               ...
               if(response)
              {
                    ...
                    if(IsValidDynamicObject( DynamicObjectID[playerid]) )
                    {
                        ...
                        ExampleAttachedDynamic = CreateDynamicObject(18649, VehPos[0], VehPos[1], VehPos[2] +1, rx, ry, rz);
  ...
  if(dialogid == DIALOG_SHOW_GPS) //If Dialog GPS data offering
    {
              ...
              if(!response)
              {
                    ...
                    LocalMessage(ACTION_DISTANCE, playerid, COLOR_ME, "* You can see the The GPS has put in his pocket.((Staff of Cityes))");
  ...
  if(dialogid == DIALOG_INPUT_BID_AUCTION) //If Dialog auctiong data offering
    ...
    new string[140];
      ...
      LocalMessage(ACTION_DISTANCE, playerid, COLOR_ME, string);
      LocalMessage(ACTION_DISTANCE, playerid, COLOR_ME, "* The stock market has changing bid.((Staff of Cityes))");
  ...
  if(dialogid == DIALOG_ACCEPT_AUCTION) //If Dialog auctiong data offering
    ...
    new string[140];
      ...
      LocalMessage(ACTION_DISTANCE, playerid, COLOR_ME, string);
      LocalMessage(ACTION_DISTANCE, playerid, COLOR_ME, "* The stock market has cancel.((Staff of Cityes))");
  ...
  if(dialogid == DIALOG_INPUT_offerINGAUCTION) //If Dialog auctiong data offering
    ...
    new string[140];
      ...
      LocalMessage(ACTION_DISTANCE, playerid, COLOR_ME, string);
      LocalMessage(ACTION_DISTANCE, playerid, COLOR_ME, "* The stock market has slain.((Staff of Cityes))");
  ...
  if(dialogid == DIALOG_GANTI_PW) //If Dialog is our register dialog
    {
      ...
      LocalMessage(ACTION_DISTANCE, playerid, COLOR_ME, string);
      LocalMessage(ACTION_DISTANCE, playerid, COLOR_ME, "* Anda dapat melihat kondisi lingkungan yang aman dan tentram.((Staff Kota))");
  ...
  if(dialogid == DIALOG_PROSSES_IKLAN) //If Dialog is our register dialog
    {
      if(!response)
      {
        ...
        LocalMessage(ACTION_DISTANCE, playerid, COLOR_ME, string);
          LocalMessage(ACTION_DISTANCE, playerid, COLOR_ME, "* Anda dapat melihat iklan dibatalkan karena malasah keuangan dari penulisnya.((Staff SANews))");
      }
      ...
      if(response)
      {
        ...
        LocalMessage(ACTION_DISTANCE, playerid, COLOR_ME, string);
          LocalMessage(ACTION_DISTANCE, playerid, COLOR_ME, "* Anda dapat melihat iklan telah di update di papan reklame bandara Kendari.((Staff SANews))");
  ...
  if(dialogid == 9) //If Dialog is our register dialog
    {
      ...
      case 0:
      {
        if(GetPlayerMoney(playerid) <200)
                {
                  SendClientMessage(playerid, COLOR_RED, "* Sir belum cukup uang untuk item ini!");
                }
  ...
  */
    ...
    if(dialogid == PILIHAN_LAMPU) //If Dialog is our register dialog
    {
      ...
      if(response) 
      {
        SendClientMessage(playerid, COLOR_YELLOW, ""COL_LIGHTBLUE"VEHICLE: "COL_WHITE"Lights has been turned on sir.");
        if(vid != INVALID_VEHICLE_ID) 
        {
            GetVehicleParamsEx(vid,engine,lights,alarm,doors,bonnet,boot,objective);
  ...
  if(dialogid == 2) //If Dialog is our register dialog
    ...
    {
      //ShowPlayerDialog(playerid, 1, DIALOG_STYLE_PASSWORD, "Login","Nama yang Sir pakai sudah terdaftar\n{FF0000} Please enter a password between 0 and 68 characters","Login","Cancel");
      ...
      //SetPlayerVirtualWorld(playerid, 0);
          //PindahinOrangnya(playerid, 1557.3281,-2260.5244,13.5407);
          SendClientMessage(playerid, COLOR_SPRINGGREEN, "Sabar ya Sir harap untuk cocokan akun masuk.");
          new yangdiinput[600];
          new GetDataFromPlayerConnect[500];
          //format(masukpw, 500, "UPDATE `playerdata` SET `money` = '%s', `password` = '%s' WHERE `playerdata`.`id` = %d;", params, pInfo[playerid][ID] );
        //mysql_query(g_Sql, masukpw);
...
public OnPlayerPickUpPickup(playerid, pickupid )
{
  ...
  if(pickupid == pintumasukrumahsakitsfpickup)
  {
    GangZoneShowForPlayer(playerid, BlackRadar, 0x000000FF);
      PlayerPlaySound(playerid, 2, 0.0, 0.0, 10.0);
    ShowPlayerDialog(playerid,0,DIALOG_STYLE_MSGBOX,"Rumah Sakit","Maaf Sir rumah sakit sedang kehabisan obat\nHarap untuk menjaga kesehatan mandiri dulu","OK","");
...
public OnPlayerEnterCheckpoint(playerid)
{
    ...
    if(IsPlayerInAnyVehicle(playerid) )
    {
                                ...
                                if(GetVehicleModel(GetPlayerVehicleID(playerid)) == 407)
                                {
                                              ...
                                              GameTextForPlayer(playerid, "~g~Mengisi air...", 2000, 5);
                                              SendClientMessage(playerid,COLOR_YELLOW,"*Matikan api dan kembali ke pangkalan.");
                                              CreateExplosion(798.45123, -1689.74084, 1
