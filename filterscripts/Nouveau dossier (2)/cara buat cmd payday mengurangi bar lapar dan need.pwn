...
CMD:payday(playerid, params[])
{
    ...
    GivePlayerMoney(playerid, pInfo[playerid][pMoney]); // menghindari bug visual untuk show money bar.
    new Float:NilaiLapar = Float:GetPlayerProgressBarValue(playerid, PlayerBar:pInfo[playerid][pLaparBar]) - Float:20.0; // kan nilai maksimum bar itu fload 100.0
    SetPlayerProgressBarValue(playerid, PlayerBar:pInfo[playerid][pLaparBar], Float:NilaiLapar);
    new string[250];
    format(string, sizeof(string), "* Anda di dompet %s di beri bonus PayDay sebesar "COL_GREEN"$%i,"COL_LIGHTBLUE" Oleh Penyumbang Pemerintah Kota", GetRPName(playerid), BantuanPayDay);
